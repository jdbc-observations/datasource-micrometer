...
NOTE: When tracking of `ResultSet` operations is disabled (see <<how-to-disable-tracking-of-resultset-operations>>), the `jdbc.generated-keys` tag shows an empty value.
...
However, in rare cases where a `ResultSet` is kept open for a long time and accessed heavily, this may lead to memory pressure.
...
[[how-to-fix-existingvalue-error]]
=== How to fix “ExistingValue must be an instance of com.zaxxer.hikari.HikariDataSource” error
...
For background and details, see <<using-features-hikaricp-support-spring-cloud-refresh>>.
