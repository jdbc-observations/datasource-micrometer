...
By default, the property is set to include(observe) all(`CONNECTION`, `QUERY`, `FETCH`) types.
...
If you want to modify the query string in the span (high cardinality tag), use `ObservationFilter` to update the tag value.
For example, you could perform query sanitization, truncation, etc.
