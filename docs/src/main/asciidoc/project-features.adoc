[[features]]
[[project-features]]
= Datasource Micrometer Features
...
Datasource Micrometer pushes the sampling decision down to the tracer implementation.
...
The following example shows setting baggage values using Datasource Micrometer's API:
...
Datasource Micrometer integrates with the OpenZipkin Brave tracer via the bridge that is available in the `datasource-micrometer-brave` module.
...
@Configuration(proxyBeanMethods = false)
	class Config {
include::{tests_path}/brave/datasource-micrometer-instrumentation-mvc-tests/src/test/java/org/springframework/cloud/sleuth/brave/instrument/web/TraceFilterWebIntegrationTests.java[tags=custom_server_sampler,indent=2]
}
----
...
Datasource Micrometer provides various integrations with the https://zipkin.io[OpenZipkin] distributed tracing system.
Regardless of the chosen tracer implementation it's enough to add `datasource-micrometer-zipkin` to the classpath to start sending spans to Zipkin.
...
Datasource Micrometer supports sending traces to multiple tracing systems as of version 2.1.0. In order to get this to work, every tracing system needs to have a `Reporter<Span>` and `Sender`.
...
Datasource Micrometer comes with a `traces` Actuator endpoint that can store finished spans. The endpoint can be queried either via an HTTP Get method to simply retrieve the list of stored spans or via HTTP Post method to retrieve the list and clear it.
