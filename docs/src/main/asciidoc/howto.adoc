...
----
    ...
    <dependency>
        <groupId>org.springframework.cloud</groupId>
        <artifactId>datasource-micrometer-zipkin</artifactId>
    </dependency>
----
...
dependencies {
    implementation "org.springframework.cloud:spring-cloud-starter-sleuth"
    implementation "org.springframework.cloud:datasource-micrometer-zipkin"
}
----
====
...
If you want Sleuth over RabbitMQ, add the `spring-cloud-starter-sleuth`, `datasource-micrometer-zipkin` and `spring-rabbit` dependencies.
...
If you want Sleuth over ActiveMQ, add the `spring-cloud-starter-sleuth`, `datasource-micrometer-zipkin` and `activemq-client` dependencies.
...
* <<missing-dependency,You have forgotten to add the dependency to report to an external system (e.g. `datasource-micrometer-zipkin`)>>
...
Up till Sleuth 3.0.0 the dependency `spring-cloud-starter-zipkin` included the `spring-cloud-starter-sleuth` dependency and the `datasource-micrometer-zipkin` dependency.
With 3.0.0 `spring-cloud-starter-zipkin` was removed, so you need to change it to `datasource-micrometer-zipkin`.
...
In case of lack of instrumentation capabilities please file https://github.com/spring-cloud/datasource-micrometer/issues[an issue] with a request to add such instrumentation.
...
Datasource Micrometer API contains all necessary interfaces to be implemented by a tracer.
